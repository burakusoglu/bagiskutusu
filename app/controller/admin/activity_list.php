<?php
 ...
 $sorgu = $db->select('activity')
         ...
         if (post('sil')) {
            ...
            ->where('activityID',$sil)
            ->run(true);
           $followMailData = $db->select('follow')
                                ->where('activityID',$sil)
                                ->run();
            if($followMailData){
              foreach ($followMailData as $value) {
                                  ...
                                  <tr>
                                      <td>
                                          <div style="margin:0 40px;padding:40px 0;border-bottom:1px solid #d3d3d3;max-width:600px!important;line-height:1.2;word-break:break-word;color:#4a4a56;font-size:16px">
                                              ...
                                              <br>
                                              <br>
                                              <b style="font-size:18px;font-weight:bold">  '. $activity['title'] .' adlı etkinlik iptal olmuştur. </b>
                                              <br>
                                              <br>
                                              <br>
                ...
                $mControl = $newMailSend->mailSend("Etkinlik Bildirimi",$content,$value['mail'],$value['name']);
                ...
                if (!$sil) {
                  ...
                  if($followMailData){
                    $followDataDelete = $db->delete('follow')
                    ->where('activityID',$sil)
                    ->done();
                  }
                  
                 
                    $delete= $db->delete('activity')
                    ->where('activityID',$sil)
                    ->done();
                    if ($delete) {
                      header("Refresh: 1;");
 ...
 require view('admin/activity_list');
 ?>
